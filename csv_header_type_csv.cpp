...
/*
用来处理 csv文件到csv文件，保持原来数据集不变，仅识别出数据条数，增加头部描述信息,形成新的带数据类型的文件
*/
class CSVReader
{
    std::string filename;
...
int main(int argc, char *argv[])
{
    ...
    bool type = false;
    for(int i = 0; i < 6 && !reader.isEmpty(); i++) {
        reader.nextLine();
    }
    ...
    std::ofstream ofs1;
    ofs1.open(rawname + "-"+ (type ? "float":"int") + "8.csv", ios::out | ios::trunc);
    if(ofs1.fail()) 
    ...
    for(int i = 0; i < 5 && !reader1.isEmpty(); i++) {
        reader1.nextLine();
    }
    if(!reader1.isEmpty())
